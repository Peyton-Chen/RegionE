...
if __name__ == "__main__":
    ...
    parser.add_argument("--guidance_scale", type=float, default=6.0, help="Guidance scale for the model")
    # config for RegionE
    ...
    parser.add_argument("--threshold", type=float, default=0.88, help="Threshold for adaptive region partition")
    parser.add_argument("--cache_threshold", type=float, default=0.02, help="Threshold for adaptive velocity decacy cache")
    parser.add_argument("--erosion_dilation", action='store_true', help="Whether to use dilation and erosion")
    # config for path
    parser.add_argument("--model_path", type=str, default="/mnt/jfs-test/lib/Step1X-Edit-v1p1-diffusers", help="Path to the pre-trained model")
    ...
    parser.add_argument("--output_dir", type=str, default="result/Step1X-Edit/Demo/RegionE", help="Directory to save the output images")
    ...
    if not args.evaluation:
        ...
        for task in os.listdir(args.image_path):
            ...
            for idx, data in enumerate(metadata):
                input_image = load_image(f'{image_path}/img/{data["key"]}.png').convert("RGB")
                prompt = data['instruction']
